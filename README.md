...
|-Bm[NumTemp]|黒のプレーヤーを内蔵思考ルーチンにセットします｡(NumTemp値の10の位以下)で指定したtemperatureに基づいて､ランダムな手を(NumTemp値の100の位以上の数+1)回打ちます｡NumTemp値を指定しなかった場合はランダムな手を打ちません｡|
|-Bx[IPaddressPort]|黒のプーレーヤーを[IPアドレス]:[ポート番号]形式で指定した[IPaddressPort]で待ち受ける外部試行ルーチンにセットします｡|
|-Wu|白のプレーヤーを人間にセットします｡|
|-Wm[NumTemp]|白のプレーヤーを内蔵思考ルーチンにセットします｡(NumTemp値の10の位以下)で指定したtemperatureに基づいて､ランダムな手を(NumTemp値の100の位以上の数+1)回打ちます｡NumTemp値を指定しなかった場合はランダムな手を打ちません｡|
...
|-Cb[value]|試合終了後､初局黒の勝率をexit codeで報告します｡勝率が(value値/10)パーセント以上となった場合､progress barの色を変えます｡|
|-Cw[value]|試合終了後､初局白の勝率をexit codeで報告します｡勝率が(value値/10)パーセント以上となった場合､progress barの色を変えます｡|
|-p[place]|盤面を表示する位置を[place]にします｡[place]値として1～3が指定可能です｡|
|-a[p]|オプション"-R[num]"指定時､指定したp[パーセント]の勝率を達成できないと確定した時点で評価を打ち切ります｡また繰返し回数が全体の半数以上を超えかつ残試合数が10試合以上残っている場合､これまでの2倍のペースで今後勝ち進んだとしても指定したp[パーセント]を達成できない場合は､評価を打ち切ります(残試合数が10未満の場合は評価を継続します)｡|
